...
class ProduitDescriptifExcursionController extends Controller
{
    /**
    ...
    public function store(StoreProduitDescriptif $request)
    {
        ...
        if ($request->ajax()) {
            return ['redirect' => url('admin/excursion-produit-descriptifs/' . $sanitized['model_id']), 'message' => trans('brackets/admin-ui::admin.operation.succeeded')];
        }

        return redirect('admin/excursion-produit-descriptifs/' . $sanitized['model_id']);
    }
    ...
    {
        $this->authorize('admin.produit-descriptif.show', $excursion);
        ...
        return view('admin.excursion.produit-descriptif.index', [
