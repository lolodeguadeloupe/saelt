...
class UpdateTarifTransfertVoyage extends FormRequest
{
    ...
    {
        return Gate::allows('admin.tarif-transfert-voyage.edit', $this->tarifTransfertVoyage);
    }
    ...
    public function getValidatorInstance()
    {
        ...
        foreach ($this->request->all() as $key => $value) {
            if (preg_match('/^type_personne_id_+\d/i', $key)) {
                $array_type_personnne_id[] = $value;

                $array_marge_aller[] = $this->{'marge_aller_' . $value};
                $array_prix_achat_aller[] = $this->{'prix_achat_aller_' . $value};
                $array_prix_vente_aller[] = $this->{'prix_vente_aller_' . $value};
                //
                $array_marge_aller_retour[] = $this->{'marge_aller_retour_' . $value};
                $array_prix_achat_aller_retour[] = $this->{'prix_achat_aller_retour_' . $value};
                $array_prix_vente_aller_retour[] = $this->{'prix_vente_aller_retour_' . $value};
            }
        }
