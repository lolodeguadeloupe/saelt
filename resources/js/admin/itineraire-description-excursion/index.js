...
Vue.component('itineraire-description-excursion-listing', {
    mixins: [AppForm],
    data: function() {
        return {
            form: { description: '' },
    ...
    methods: {
        editDescription(event, url) {
            var _this5 = this;
            let loading = $(event.target).InputLoadRequest();
            axios.get(url).then(function(response) {
                _this5.is_edit = true;
                _this5.form = { description: response.data && response.data.description ? response.data.description : "" };
            }, function(error) {
        ...
        saveDescription(event, url) {
            var _this5 = this;
            let loading = $(event.target).InputLoadRequest();
            axios.post(url, {
                description: _this5.form.description,
                excursion_id: `${_this5.model.id}`
