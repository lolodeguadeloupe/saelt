...
Vue.component('detail-service-port-listing', {
    ...
    methods: {
        editService(event, url) {
            ...
            axios.get(url).then(function(response) {
                _this5.actionEditService = response.data.servicePort.resource_url;
                _this5.actionCalendar = response.data.servicePort.resource_url + '/calendar';
                _this5.logo = response.data.servicePort.logo ? [response.data.servicePort.logo] : [];
                _this5.$modal.show('edit_service');
                _this5.myEvent = {
                    ..._this5.myEvent,
                    'editService': function() {
                        var _data = response.data.servicePort
        ...
        storeService(event, modal) {
            ...
            this.mySubmit(event, {
                success: function(response) {
                    ...
                    _this5.data_detail = [{...(response.data.servicePort ? response.data.servicePort : []) }];
                    _this5.globalCalendarDates = response.data.servicePort.calendar;
                }
            }, false, {
