...
Vue.component('hebergement-listing', {
    ...
    data: function() {
        return {
            ...
            types: [],
            myEvent: {},
            form_heb: [false, false],
            form_heb_edit: [false, false],
    ...
    methods: {
        editHeb(event, url, tabs = [false, true], tab_action = false) {
            var _this5 = this;
            this.form_heb_edit = tabs.map(tab => !tab);
            this.url_resourse_heb = url;
            _this5.imageUploads = [];
            _this5.imageServeur = [];
            ...
            axios.get(url).then(function(response) {
                ...
                if (tab_action) {
                    $('#edit-hebergement').each(function() {
                        Myform(this).setForm({
                            ...response.data.hebergement,
                            ville_name: response.data.hebergement.ville.name,
                            ile_name: response.data.hebergement.ile.name
                        });
                    })
                } else {
        ...
        editPrestataire(event, url, tabs) {
            ...
            let loading = $(event.target).InputLoadRequest();
            axios.get(url).then(function(response) {
                _this5.actionEditPrestataire = `${_this5.actionEditHeb}/prestataire/${response.data.prestataire.id}`;
        ...
        createHeb(event, url) {
            ...
            axios.get(url).then(function(response) {
                _this5.types = response.data.type_hebergements;
                _this5.taxes = response.data.taxe;
                _this5.form_heb = [true, false];
                _this5.$modal.show('create_hebergement');
            }, function(error) {
                _this5.$notify({ type: 'error', title: 'Error!', text: error.response.data.message ? error.response.data.message : 'An error has occured.' });
            }).finally(() => {
                loading.remove();
                            ...
                            errors: function(error) {
                                $('#create-hebergement').each(function() {
                                    if (error.response && error.response.status && error.response.status === 422) {
                                        ...
                                        for (const xx in origin_errors) {
                                            ...
                                            if (x.length > 1 && x[0] == "heb") {
                                                new_error[x[1]] = origin_errors[xx];

                                            }
                                        }
                                        Myform(this).erreur({ error: 422, data: new_error });
                                    }
                                })
                                $('#create-prestataire').each(function() {
                                    if (error.response && error.response.status && error.response.status === 422) {
                                        ...
                                        for (const xx in origin_errors) {
                                            const x = String(xx).split('.');
                                            if (x.length > 1 && x[0] == "prest") {
                                                new_error[x[1]] = origin_errors[xx];

                                            }
                                        }
                                        Myform(this).erreur({ error: 422, data: new_error });
                                        for (var err in new_error) {
                                            if (['name', 'phone', 'adresse', 'ville_id', 'description'].indexOf(String(err)) >= 0 && !_this5.form_heb[0])
                                                _this5.form_heb = [true, false];
                                        }
                                    }
                                })
        ...
        updateHeb(event, modal) {
            var _this5 = this;
            this.mySubmit(event, {
                success: function(response) {
                    _this5.form_heb = [false, true];
                    _this5.calendarLocal = [];
                    _this5.modelCalendar = [];
                    _this5.calendarServeur = response.data.calendar;
        ...
        changeFormHeb(form = []) {
            var _this5 = this;
            if (_this5.form_heb[0]) {
                $('#create-prestataire').each(function() {
                    if (Myform(this).serialized()) {
                        _this5.form_heb = form.map(is => !is);
                    }
                })
            } else {
                _this5.form_heb = form.map(is => !is);
        ...
        autocomplete() {
            ...
            return {
                ajax: axios,
                fnSelectItem: function(item) {
                    _this5.actionEditPrestataire = `${_this5.actionEditHeb}/prestataire/${item.id}`;
                            ...
                            _this5.eventFormSubmit = {
                                ...
                                create_prestataire: {
                                    ...
                                    fnSet: function(form, data) {
                                        ...
                                        _this5.actionEditPrestataire = `${_this5.actionEditHeb}/prestataire/${data.id}`;
                                    }
                                }
                            }
                        }, function(error) {
