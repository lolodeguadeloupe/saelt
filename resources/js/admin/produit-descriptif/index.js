...
Vue.component('produit-descriptif-listing', {
    mixins: [AppForm],
    data: function() {
        return {
            form: { descriptif: '' },
    ...
    methods: {
        editDescriptif(event, url) {
            var _this5 = this;
            let loading = $(event.target).InputLoadRequest();
            axios.get(url).then(function(response) {
                _this5.is_edit = true;
                _this5.form = { descriptif: response.data && response.data.descriptif ? response.data.descriptif : "" };
            }, function(error) {
        ...
        saveDescriptif(event, url) {
            var _this5 = this;
            let loading = $(event.target).InputLoadRequest();
            axios.post(url, {
                descriptif: _this5.form.descriptif,
