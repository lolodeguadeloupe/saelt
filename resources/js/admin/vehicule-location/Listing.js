...
Vue.component('vehicule-location-listing', {
    ...
    data: function() {
        return {
            ...
            actionCalendar: null,
            myEvent: {},
            form_vehicule: [false, false, false],
            form_vehicule_edit: [false, false, false],
            url_resourse_prestataire: "",
            url_resourse_vehicule: "",
    ...
    methods: {
        prewiewFile(_file_base64) {
            const arr_file = String(_file_base64).split(';');
            return `${arr_file[1].split('base64,')[1]}`;
        },
        ...
        editVehicule(event, url, tabs = [true, false, false], tab_action = false) {
            var _this5 = this;
            this.form_vehicule_edit = tabs;
            this.url_resourse_vehicule = url;
            _this5.imageUploads = [];
            _this5.imageServeur = [];
            ...
            axios.get(url).then(function(response) {
                ...
                _this5.url_resourse_prestataire = response.data.vehiculeLocation.prestataire.resource_url;
                _this5.url_resourse_fiche_technique = response.data.vehiculeLocation.info_tech.resource_url;
                ...
                _this5.ficheTechnique = response.data.vehiculeLocation.info_tech.fiche_technique ? [{
                    fiche_technique: response.data.vehiculeLocation.info_tech.fiche_technique,
                    fiche_technique_b64: response.data.vehiculeLocation.info_tech.fiche_technique_b64
                }] : [];
                const data = {
                ...
                if (tab_action) {
                    ...
                    $('#create-info-tech').each(function() {
                        Myform(this).setForm(data.info_tech);
                    })
                } else {
                    ...
                    _this5.myEvent = {
                        ...
                        'editVehicule': function() {
                            ...
                            $('#create-info-tech').each(function() {
                                Myform(this).setForm(_data.info_tech);
                            })
                        }
                    }
                }
        ...
        editPrestataire(event, url, tabs) {
            ...
            let loading = $(event.target).InputLoadRequest();
            axios.get(url).then(function(response) {
                _this5.actionEditPrestataire = `${_this5.actionEditVehicule}/prestataire/${response.data.prestataire.id}`;
        ...
        editFicheTechnique(event, url, tabs) {
            var _this5 = this;
            this.form_vehicule_edit = tabs;
            let loading = $(event.target).InputLoadRequest();
            axios.get(url).then(function(response) {
        ...
        createVehicule(event, url) {
            ...
            _this5.logo = [];
            axios.get(url).then(function(response) {
                _this5.form_vehicule = [true, false, false];
        ...
        storeVehicule(event, modal) {
            var _this5 = this;
            const data_vehicule = _this5.submitVehiculeForm();
            const data_prest = _this5.submitPrestataireForm();
            const data_info_tech = _this5.submitInfoTechForm();
            if (data_vehicule && data_prest && data_info_tech) {
                var loading = $(event.target).InputLoadRequest();
                $('#create-vehicule').each(function() {
                    _this5.postData(
                        _this5.urlvehiculelocationwithprestataire, {
                            vehicule: {
                                ...data_vehicule,
                            ...
                            errors: function(error) {
                                if (error.response && error.response.status && error.response.status === 422) {
                                    ...
                                    var errorsVehicule = {};
                                    var errorsPrestataire = {};
                                    var errrosInfoTech = {};
                                    for (const xx in origin_errors) {
                                        const x = String(xx).split('.');
                                        if (x.length && x[0] == "technique") {
                                            errrosInfoTech[x[1]] = origin_errors[xx];
                                            _this5.form_vehicule = [false, false, true];
                                        } else if (x.length && x[0] == "vehicule") {
                                            errorsVehicule[x[1]] = origin_errors[xx];
                                            _this5.form_vehicule = [false, true, false];
                                        } else if (x.length && x[0] == "prest") {
                                            errorsPrestataire[x[1]] = origin_errors[xx];
                                            _this5.form_vehicule = [true, false, false];
                                        }
                                    }
                                    $('#create-vehicule').each(function() {
                                        Myform(this).erreur({ error: 422, data: errorsVehicule });
                                    })
                                    $('#create-prestataire').each(function() {
                                        Myform(this).erreur({ error: 422, data: errorsPrestataire });
                                    })
                                    $('#create-info-tech').each(function() {
                                        Myform(this).erreur({ error: 422, data: errorsPrestataire });
                                    })
        ...
        updateVehicule(event, modal) {
            ...
            this.mySubmit(event, {
                success: function(response) {
                    ...
                    _this5.modelCalendar = [];
                    _this5.calendarServeur = [];
                    _this5.editFicheTechnique(event, `${_this5.url_resourse_fiche_technique}/edit`, [false, true, false]);
                }
            }, false, {
                image: _this5.imageUploads,
                calendar: _this5.calendarLocal,
        ...
        updateInfoTech(event, modal) {
            var _this5 = this;
            this.mySubmit(event, {
                success: function(response) {
                    _this5.editPrestataire(event, `${_this5.url_resourse_prestataire}/edit`, [false, false, true]);
                    _this5.$modal.hide(modal);
                }
            }, false, {}, true);
        },
        ...
        changeFormVehicule(form = []) {
            var _this5 = this;
            if (_this5.form_vehicule[0]) {
                $('#create-prestataire').each(function() {
                    if (Myform(this).serialized()) {
                        _this5.form_vehicule = [...form];
                    }
                })
            } else if (_this5.form_vehicule[1]) {
                $('#create-vehicule').each(function() {
                    if (Myform(this).serialized()) {
                        _this5.form_vehicule = [...form];
                    }
        ...
        autocompletePrestataire() {
            ...
            return {
                ajax: axios,
                fnSelectItem: function(item) {
                    _this5.actionEditPrestataire = `${_this5.actionEditVehicule}/prestataire/${item.id}`;
                            ...
                            _this5.eventFormSubmit = {
                                ...
                                create_prestataire: {
                                    ...
                                    fnSet: function(form, data) {
                                        ...
                                        _this5.actionEditPrestataire = `${_this5.actionEditVehicule}/prestataire/${data.id}`;
                                    }
                                }
                            }
                        }, function(error) {
