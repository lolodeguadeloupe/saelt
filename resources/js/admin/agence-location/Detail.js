...
Vue.component('detail-agence-location-listing', {
    ...
    computed: {
        ...
        modelcalendardates() {
            return JSON.stringify(this.modelCalendar.map(data => {
                return moment(new Date(data.attrs.dates)).set({ 'hour': 0, 'minute': 0, 'seconde': 0, 'millisecond': 0 }).format('YYYY-MM-DD');
            }));
        },
        calendaralldates() {
            var _this5 = this;
    ...
    methods: {
        editAgence(event, url) {
            var _this5 = this;
            _this5.calendarLocal = []; //init calendar local
            let loading = $(event.target).InputLoadRequest();
            axios.get(url).then(function(response) {
                _this5.actionEditAgence = response.data.agenceLocation.resource_url;
                _this5.actionCalendar = response.data.agenceLocation.resource_url + '/calendar';
                _this5.logo = response.data.agenceLocation.logo ? [response.data.agenceLocation.logo] : [];
                _this5.$modal.show('edit_agence');
                _this5.myEvent = {
                    ..._this5.myEvent,
                    'editAgence': function() {
                        var _data = response.data.agenceLocation
                        $('#edit-agence').each(function() {
        ...
        storeAgence(event, modal) {
            ...
            this.mySubmit(event, {
                success: function(response) {
                    ...
                    _this5.data_detail = [{...(response.data.agenceLocation ? response.data.agenceLocation : []) }];
                    _this5.globalCalendarDates = response.data.agenceLocation.calendar;
                }
            }, false, {
