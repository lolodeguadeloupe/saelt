...
Vue.component('ile-listing', {
    ...
    methods: {
        editIle(event, url) {
            var _this5 = this;
            _this5.cardImage = [];
            _this5.backgroundCardImage = [];
            let loading = $(event.target).InputLoadRequest();
            axios.get(url).then(function(response) {
                _this5.actionEditIle = response.data.ile.resource_url;
                _this5.cardImage = response.data.ile.card ? [response.data.ile.card] : [];
                _this5.backgroundCardImage = response.data.ile.background_image ? [response.data.ile.background_image] : [];
                _this5.$modal.show('edit_ile');
                _this5.myEvent = {
                    ..._this5.myEvent,
                    'editIle': function() {
                        var _data = response.data.ile
                        $('#edit-ile').each(function() {
        ...
        uploadBackGroundCart(event, drop = false) {
            var _this5 = this,
                progress_bar = $("<div/>").css({
                    'background-color': 'green',
                    'width': '0%',
            ...
            for (const file of files) {
                if (file && file.type && ['image/jpg', 'image/jpeg', 'image/png', 'image/gif'].includes(String(file.type))) {
                    if (file.size && file.size > 1024 * 1024 * 1024 * 4) {
                        ...
                        var reader = new FileReader();
                        reader.onload = (data) => {
                            _this5.backgroundCardImage = [data.target.result];
                            progress_bar.remove();
                        }
                        reader.onprogress = (_event) => {
                            $($(event.target).is('figure') ? $(event.target) : $(event.target).parents('figure')).each(function() {
