(function ($) {
        ...
        if (typeof returns === 'string') {
            if (returns === 'this')
                ...
                var p = $(this).parent(),
                    left = 0,
                    top = 0,
                    eps_left = parseInt($(this).offset().left) - parseInt(p.offset().left),
                    eps_top = parseInt($(this).offset().top) - parseInt(p.offset().top)
                do {
    ...
    var _removeInputErrer = function () {
        ...
        $(`[name="${$(my_controle).attr('name')}"]`).each(function () {
            if (!$(my_controle).is(this)) {
                $(this).each(function () {
                    $(this).dettachClass('form-control-danger').attachClass('form-control-success').nextAll('div.form-control-feedback').each(function () {
                        $(this).remove();
                    });
    ...
    var _controleInput = function () {
        var element = $(this),
            dd = function (val) {
                const signe = String(val).search(/\-/) == 0;
                var separator = [' ', '-'],
                    resulta = val;
                var count_separer = 0;
                separator.map(function (_separator) {
                    var i = resulta.split(_separator);
                    if (i.length == 1) return;
                    for (var j = 0; j < i.length; j++) {
                        resulta = j == 0 ?
                            $.trim(i[j]) !== '' ? $.trim(i[j]).substr(0, 1).toUpperCase() + $.trim(i[j]).substr(1).toLowerCase() : '' :
                            (resulta !== '' ?
                                resulta += _separator + ($.trim(i[j]) !== '' ? $.trim(i[j]).substr(0, 1) + $.trim(i[j]).substr(1).toLowerCase() : '') :
                                $.trim(i[j]) !== '' ? $.trim(i[j]).substr(0, 1).toUpperCase() + $.trim(i[j]).substr(1).toLowerCase() : '')
                        count_separer++;
                    }
                })
                if (count_separer == 0) {
                    resulta = $.trim(val).substr(0, 1).toUpperCase() + $.trim(val).substr(1).toLowerCase();
                }
                if (signe) {
                    resulta = `-${resulta}`;
                }
        ...
        element.each(function () {
            $(this).focusout(function (e) {
                ...
                if ($(this).is('input[type="email"]') || $(this).is('input[name="email"]') || $(this).is('input[type="password"]') || $(this).is('input[name="password"]') || $(this).hasAttr('custom')) {
                    $(this).val(trimPoint(e.target.value))
                    return;
                }
                if ($(this).hasClass('upercase'))
                    $(this).val(e.target.value.toUpperCase())
                else if ($(this).hasClass('cutomer-format-control')) {
                    $(this).val(trimPoint(e.target.value));
                } else
