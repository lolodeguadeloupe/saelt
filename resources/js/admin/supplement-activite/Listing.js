...
Vue.component('supplement-activite-listing', {
    mixins: [AppListing, AppForm],
    data: function () {
        return {
            form: {},
            actionEditSupActivite: '',
            actionEditTarifPersonne: '',
            chambres: [],
            myEvent: {},
            typePersonnes: [],
            icon: ['assets/img/supplement/activity.png']
        }
    },
    methods: {
        editSupActivite(event, url) {
            var _this5 = this;
            ...
            axios.get(url).then(function (response) {
                ...
                _this5.icon = response.data.supplementActivite.icon ? [response.data.supplementActivite.icon] : [];
                _this5.actionEditSupActivite = response.data.supplementActivite.resource_url;
                _this5.chambres = response.data.chambres;
                _this5.$modal.show('edit_sup_activite');
                var _data_tarif = { ...response.data.supplementActivite },
                ...
                _this5.myEvent = {
                    ..._this5.myEvent,
                    'editSupActivite': function () {
                        $('#edit-sup-activite').each(function () {
        ...
        createSupActivite(event, url) {
            var _this5 = this;
            _this5.typePersonnes = [];
            _this5.icon = ['assets/img/supplement/activity.png'];
            let loading = $(event.target).InputLoadRequest();
            axios.get(url).then(function (response) {
                _this5.typePersonnes = response.data.typePersonne ? response.data.typePersonne : [];
                _this5.chambres = response.data.chambres;
                _this5.myEvent = {
                    ..._this5.myEvent,
                    'createSupActivite': function () {
                        loading.remove()
                    }
                }
                _this5.$modal.show('create_sup_activite');
            }, function (error) {
                _this5.$notify({ type: 'error', title: 'Error!', text: error.response.data.message ? error.response.data.message : 'An error has occured.' });
        ...
        editTarifPersonne(event, url, condition) {
            ...
            let loading = $(event.target).InputLoadRequest();
            axios.get(url).then(function (response) {
                _this5.actionEditTarifPersonne = response.data.tarifSupplementActivite.resource_url;
                _this5.myEvent = {
                    ..._this5.myEvent,
                    'editTarifPersonne': function () {
                        $('#edit-tarif-form' + condition).each(function () {
                            Myform(this).setForm({ ...response.data.tarifSupplementActivite, type_personne: response.data.tarifSupplementActivite.personne.type });
                        })
                    }
                }
