...
Vue.component('all-compagnie-transport-listing', {
    ...
    methods: {
        editCompagnie(event, url) {
            var _this5 = this;
            _this5.logo = [];
            let loading = $(event.target).InputLoadRequest();
            axios.get(url).then(function(response) {
                _this5.actionEditCompagnie = response.data.compagnieTransport.resource_url;
                _this5.logo = response.data.compagnieTransport.logo ? [response.data.compagnieTransport.logo] : [];
                _this5.$modal.show('edit_compagnie');
                _this5.myEvent = {
                    ..._this5.myEvent,
                    'editCompagnie': function() {
                        var _data = response.data.compagnieTransport
                        $('#edit-compagnie').each(function() {
