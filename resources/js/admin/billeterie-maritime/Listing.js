...
Vue.component('billeterie-maritime-listing', {
    ...
    methods: {
        ...
        editBilleterie(event, url, tabs = [false, true, true], tab_action = false) {
            var _this5 = this;
            _this5.is_create = false;
        ...
        createBilleterie(event, url) {
            var _this5 = this;
            let loading = $(event.target).InputLoadRequest();
            _this5.is_create = true;
            _this5.typePersonnes = [];
            _this5.compagnies = [];
            _this5.images = [];
            _this5.parcours = 1;
            axios.get(url).then(function (response) {
                _this5.typePersonnes = (response.data.typePersonne ? response.data.typePersonne : []).map(function (_type) {
                    return { ..._type, original_id: _type.id };
                });
                _this5.compagnies = response.data.compagnieTransport ? response.data.compagnieTransport : [];
                _this5.billeterie_form = [true, false];
                _this5.$modal.show('create_billeterie');
            }, function (error) {
                _this5.$notify({ type: 'error', title: 'Error!', text: error.response.data.message ? error.response.data.message : 'An error has occured.' });
            }).finally(() => {
                loading.remove();
        ...
        storeBilleterie(event, modal) {
            ...
            $('#billeterie_maritime_block').each(function () {
                $(this).children('div').removeClass('block-error');
            ...
            this.mySubmit(event, {
                success: function (response) {
                    ...
                    _this5.url_item_time = response.data.billeterieMaritime.resource_url;
                    _this5.url_get_time = response.data.billeterieMaritime.resource_url;
                    _this5.times = response.data.billeterieMaritime.planingTime;
                    _this5.billeterie_form = [false, false, true];
                },
                errors: function (error) {
                    ['#create-billeterie', '#edit-billeterie'].map(id => {
                        $(id).each(function () {
                            if (error.response && error.response.status && error.response.status === 422) {
                                ...
                                _this5.billeterie_form = [true, false, false];
                                ...
                                $('#billeterie_maritime_block').each(function () {
                                    $(this).children('div').addClass('block-error');
        ...
        changeFormBilleterie(event, form = []) {
            ...
            var _this5 = this;
            if (_this5.billeterie_form[1] && form[2] == false) {
                var data = null;
                ['create-billeterie', 'edit-billeterie'].map(function (_form) {
                    $(`#${_form}`).each(function () {
                        data = Myform(this).serialized();
                    })
                })
                if (data != null) {
                    _this5.$refs.submitBilleterie.click();
                }

            } else if (form[2] == false) {
                _this5.getDetail();
                _this5.billeterie_form = form.map(is => !is);
            } else if (_this5.billeterie_form[0]) {
                var isValid = true;
                $('[name="titre"],[name="parcours"],[name="compagnie_transport_id"],[name="quantite"],[name="date_acquisition"][name="date_acquisition"],[name="date_limite"],[name="lieu_depart"],[name="date_depart"],[name="lieu_arrive"],[name="date_arrive"]').each(function () {
        ...
        detail(event, id) {
            ...
            this.$managerliens({
                is_parent: false,
                parent_name: 'billeterie',
                href: `${this.url}/${id}?billeterie`,
                name: 'Info',
                body: event.target,
                _this: this,
                axios: axios,
            ...
            return {
                ...
                fnBtnNew: function (event, options) {
                    _this5.$managerliens({
                        ...
                        name: _this5.$dictionnaire.aeroports,
                        body: event.target,
                        _this: _this5,
                        axios: axios,
                        liens: 'other',
        ...
        storePlaningTime(event) {
            ...
            this.mySubmit(event, {
                success: function (response) {
                    _this5.url_item_time = _this5.url_get_time;
            ...
            }, false, { ...availability, id_model: _this5.id_model }, false);
        },
        getDetail() {
            var loading = null;
            ...
            var _this5 = this;
            axios.get(_this5.url_item_time).then(function (response) {
                _this5.times = response.data.data.planingTime;
            }, function (error) {
                _this5.$notify({ type: 'error', title: 'Error!', text: error.response.data.message ? error.response.data.message : 'An error has occured.' });
        ...
        showPlaningTime(item) {
            var _this5 = this;
            _this5.id_model = `billeterie_maritime_${item.id}`;
            _this5.url_item_time = item.resource_url;
            _this5.url_get_time = item.resource_url;
            _this5.has_edit_time = false;
            _this5.has_create_time = false;
            _this5.$modal.show('create_times');
            _this5.getDetail();
        },
        createPlaningTime() {
            var _this5 = this;
