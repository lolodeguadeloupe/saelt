...
Vue.component('type-chambre-listing', {
    ...
    methods: {
        editTypeChambre(event, url) {
            ...
            axios.get(url).then(function(response) {
                _this5.actionEditType = response.data.typeChambre.resource_url;
                _this5.actionCalendar = response.data.typeChambre.resource_url + '/calendar';
                _this5.imageServeur = response.data.typeChambre.image ? [...response.data.typeChambre.image].map(function(image) { return {...image, name: `${image.name}` } }) : [];
                _this5.$modal.show('edit_type');
                _this5.myEvent = {
                    ..._this5.myEvent,
                    'editType': function() {
                        var _data = response.data.typeChambre,
                            _this55 = _this5;
        ...
        createType(event, url) {
            ...
            _this5.imageServeur = [];
            _this5.calendarLocal = []; //init calendar local
            _this5.actionCalendar = _this5.urlbase + '/admin/type-chambres/0/calendar';
            let loading = $(event.target).InputLoadRequest();
            axios.get(url).then(function(response) {
                ...
                _this5.myEvent = {
                    ..._this5.myEvent,
                    'createType': function() {
                        var _data = { 'hebergement_id': _this5.hebergement.id };
        ...
        storeType(event, modal) {
            ...
            this.mySubmit(event, {
                success: function(response) {
                    ...
                    _this5.calendarServeur = response.data.typeChambre.calendar;
                    _this5.$modal.hide(modal);
                }
            }, false, { image: _this5.imageUploads, calendar: _this5.calendarLocal });
        },
        closeModal(modal) {
        ...
        afficherCalendar(event, url) {
            var _this5 = this;
            let loading = $(event.target).InputLoadRequest();
            axios.post(url).then(function(response) {
                _this5.$modal.show('calendar-all-date');
                _this5.calendar_all_dates = response.data.calendar.map(calendar => {
                        ...
                        case 'bar':
                            attrs = {
                                bar: {
                                    style: {
                                        backgroundColor: calendar.color,
                                        class: Date.now() > calendar.date ? 'opacity-75' : '',
                                    }
                                },
                            }
                            break;
                    ...
                    return {
                        ...
                        dates: moment(new Date(calendar.date)).set({ 'hour': 0, 'minute': 0, 'seconde': 0, 'millisecond': 0 }).toDate(),
                    }
                });
            }, function(error) {
                _this5.$notify({ type: 'error', title: 'Error!', text: error.response.data.message ? error.response.data.message : 'An error has occured.' });
