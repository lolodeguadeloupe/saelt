    ...
    <div class="bloc-info p-relative">
        ...
        <div class="body-prestataire p-relative v-top table">
            ...
            <div class="info-prest-content d-relative t-cell v-top">
                <span class="value d-block v-top">{{$data['compagnie']['compagnie_nom']}} </span>
                <span class="value d-block v-top">{{$data['compagnie']['compagnie_adresse']}} {{$data['compagnie']['compagnie_ville']}} - {{$data['compagnie']['compagnie_code_postal']}}</span>
                <span class="value d-block v-top">{{$data['compagnie']['compagnie_phone']}}</span>
            </div>
            <div class="info-prest-history d-relative t-cell v-top">
                <div class="row">
                    <p><span class="title">Date d'émission :</span><br> {{parse_date_string(date('Y-m-d'))}}</p>
        ...
        <div class="detail-voucher p-relative table v-top border">
            <div class="voucher-col1 t-cell v-top p-relative">
                ...
                <div class="row d-block t-left v-top p-5">
                    <div class="row d-block"><span class="title">SERVICES :</span> {{ trans('admin.excursion.columns.options_included.ticket') }}</div>
                    <div class="row d-block"><span class="title">DATE DEPART :</span> {{parse_date_string($data['compagnie']['billet_date_depart'])}} à {{date('H:i', strtotime($data['heure_depart']))}}</div>
                    <div class="row d-block"><span class="title">LIEU DEPART :</span>{{$data['compagnie']['billet_lieu_depart_name']}}</div>
                    <div class="row d-block"><span class="title">LIEU ARIVEE :</span>{{$data['compagnie']['billet_lieu_arrive_name']}}</div>
                    <div class="row d-block"><span class="title">PASSAGERS :</span>
                        @php
                        $count_passage = 0;
                        foreach($data['personne'] as $_pers){
                        $count_passage = $count_passage + intval($_pers['nb']);
                        }
                        @endphp
                        {{$count_passage}}
                    </div>
            ...
            <div class="voucher-col2 t-cell v-top p-relative border b-t-t b-b-t">
                ...
                <div class="row p-relative t-center">
                    Compagnie-{{$data['compagnie']['compagnie_id']}}
                </div>
            </div>
