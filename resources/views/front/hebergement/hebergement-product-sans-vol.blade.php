...
<hebergement-host-detail-sans-vol :url="'{{route('hebergement-product-sans-vol')}}'" :urlbase="'{{base_url('')}}'" :urlasset="'{{asset('')}}'" :data="{{$data->toJson()}}" :aside="{{$aside}}" :sessionrequest="{{$session_request}}" :keysessionrequest="'{{app('request')->has('key_')?app('request')->key_:''}}'" :personne="{{$personne->toJson()}}" :mycommande="{{$commande_saved->toJson()}}" :editpannier="'{{$edit_pannier==false?'false':'true'}}'" :chambresaison="{{$chambre_saison}}" inline-template>
	<section class="py-5">
					...
					<div class="tab-content">
										...
										<ul class="trip-overview">
											...
											<li>
												...
												<div class="detail">
													...
													<div class="desc">{{trans('front-hebergement.sans_vol')}}</div>
												</div>
											</li>
						...
						<div class="tab-pane fade show active" role="tabpanel" aria-labelledby="booking-tab" id="booking">
								...
								<div class="col-md-5 px-5 mb-5 col-calendar">
									<div class="nb-days mb-2 row">
										<div class="flex-fill col-6">
											<h5>{{trans('front-hebergement.dates_sejour')}} <span style="color:#ab2e30">*</span> :</h5>
										</div>
										<div class="flex-fill col-6">
											<p>@{{dateDebutCalendar}} - @{{dateEndCalendar}}</p>
										</div>
									</div>
									...
									<div class="calendar" id="my_calendar">
										<v-date-picker :attributes="attributes" :color="colorSelectDate" :disabled-dates="excludedDate" ref="datepicker" is-expanded v-model="calendarRange" :from-date="min_saison" :min-date='min_saison' :max-date='max_saison' is-range />
									</div>
									<div class="row mx-0 px-3 py-2 d-none">
										<div v-if="nuit_min>0" class="col font-weight-bold text-center" style="color: #ab2e30">
											{{trans('front-hebergement.min_')}} @{{nuit_min}} {{trans('front-hebergement._nuits')}}
										</div>
										<div v-if="nuit_max>0" class="col font-weight-bold text-center" style="color: #ab2e30">
											{{trans('front-hebergement.max_')}} @{{nuit_max}} {{trans('front-hebergement._nuits')}}
										</div>
									</div>
								</div>
								<div class="col-md-4 px-5 mb-5 col-pers">
									<div class="nb-pers">
										<h5>{{trans('front-hebergement.nombre_personne')}} <span style="color:#ab2e30">*</span> :</h5>
										<div class="block-input" v-for="_personne of item.tarif">
											...
											<input type="number" :name="'personne_'+_personne.personne.id" required class="form-input" step="1" data-oldValue="0" min="0" value="0" @input="checkPersonne($event,'personne_',$getKey(item.tarif,'type_personne_id'),_personne.personne.id)" @focusout="$setDefaultValue">
										</div>
										<h5>{{trans('front-hebergement.nombre_chambre')}} <span style="color:#ab2e30">*</span> :</h5>
							...
							<div class="row product-footer">
								...
								<div class="col-md-3 mb-5 btn btn-primary" v-if="nombreNuitCalendar > 0" @click.prevent="validerCommander" :id-commande="item.id" :reference-prix="item.reference_tarif.prix_vente" :titre-commande="item.type_chambre.name" :image-commande="item.type_chambre.image.length > 0? item.type_chambre.image[0].name:'/assets/img/bois-joli-bungalow.jpg'" :produit-url="url">
...
</hebergement-host-detail-sans-vol>
								...
								<div class="pb-3 pl-4 pr-4 pt-4 d-flex flex-grow-1 flex-column position-relative">
									<div class="d-flex flex-column flex-grow-1">
										...
										<div class="d-flex">
											...
											<div class="flex-fill text-center px-1 d-flex" v-if="chambre_item.tarif.type_chambre.formule" style="width: 50%;">
												<div class="font-size-0-8-em supplement position-relative d-flex align-items-center m-auto">
											...
											<div class="flex-fill text-center px-1 d-flex" v-if="chambre_item.tarif.type_chambre.formule == null" style="width: 50%;">
												<div class="font-size-0-8-em supplement position-relative d-flex align-items-center m-auto">
