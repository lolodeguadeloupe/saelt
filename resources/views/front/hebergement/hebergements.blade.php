...
<hebergement-ile :url="'{{route('hebergements')}}'" :urlbase="'{{base_url('')}}'" :urlasset="'{{asset('')}}'" :data="{{$data->toJson()}}" :aside="{{$aside}}" :sessionrequest="{{$session_request}}" :keysessionrequest="'{{app('request')->has('key_')?app('request')->key_:''}}'" inline-template>
    <section class="py-5">
        <div class="container">
            <div class="row" v-if="collection[0] != undefined">
                <div class="col-md-4 py-3" v-for="(item,index) in collection">
                    <div class="align-items-center background-center-center background-cover d-flex h-100 justify-content-center px-2 py-5" :style="'background-image: url(\''+(item.background_image? $isBase64(item.background_image)?item.background_image:`${urlasset}/${item.background_image}`: '{{asset('/assets/img/Guadeloupe.jpg')}}' )+'\')'">
                        <div>
                            <a @contextmenu.prevent="" @click.prevent="managerRequest($event,'{{ route('hebergement-all-hosts') }}',{ile:item.id})" href="#" class="text-center text-decoration-none text-white">
                                <div>
