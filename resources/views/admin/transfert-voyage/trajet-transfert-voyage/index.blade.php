...
@section('title', trans('admin.trajet-transfert-voyage.actions.index'))

@section('body')

<trajet-transfert-voyage-listing :data="{{ $data->toJson() }}" :url="'{{ url('admin/trajet-transfert-voyages') }}'" :action="'{{ url('admin/trajet-transfert-voyages') }}'" :urlbase="'{{base_url('')}}'" :urlasset="'{{asset('')}}'" inline-template>
    ...
    <div style="display: contents;position:relative">
            ...
            <div class="col">
                <div class="card position-relative">
                    <div class="card-header">
                        ...
                        <a class="btn btn-primary btn-sm pull-right m-b-0" href="#" @click.prevent="createTrajet($event,'{{ url('admin/trajet-transfert-voyages/create')}}')" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.trajet-transfert-voyage.actions.create') }}</a>
                    </div>
                    <div class="card-body" v-cloak>
                        <div class="card-block position-relative">
                                    ...
                                    <thead>
                                        ...
                                        <tr v-show="(clickedBulkItemsCount > 0) || isClickedAll">
                                            <td class="bg-bulk-info d-table-cell text-center" colspan="6">
                                                <span class="align-middle font-weight-light text-dark">{{ trans('admin-base.listing.selected_items') }} @{{ clickedBulkItemsCount }}. <a href="#" class="text-primary" @click.prevent="onBulkItemsClickedAll('/admin/trajet-transfert-voyages')" v-if="(clickedBulkItemsCount < pagination.state.total)"> <i class="fa" :class="bulkCheckingAllLoader ? 'fa-spinner' : ''"></i> {{ trans('admin-base.listing.check_all_items') }} @{{ pagination.state.total }}</a> <span class="text-primary">|</span> <a href="#" class="text-primary" @click.prevent="onBulkItemsClickedAllUncheck()">{{ trans('admin-base.listing.uncheck_all_items') }}</a> </span>

                                                <span class="pull-right pr-2">
                                                    <button class="btn btn-sm btn-danger pr-3 pl-3" @click.prevent="bulkDelete(urlbase+'/admin/trajet-transfert-voyages/bulk-destroy')">{{ trans('admin-base.btn.delete') }}</button>
                                                </span>
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <a class="btn btn-primary" href="#" @click.prevent="createTrajet($event,'{{ url('admin/trajet-transfert-voyages/create')}}')" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.trajet-transfert-voyage.actions.create') }}</a>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <modal name="edit_trajet" :scrollable="true" :min-width="320" :max-width="1000" height="auto" width="100%" :draggable="false" @opened="myEvent.editTrajet()" :adaptive="true">
            @include('admin.transfert-voyage.trajet-transfert-voyage.edit')
        </modal>
        ...
        <modal name="create_trajet" :click-to-close="false" :scrollable="true" :min-width="320" :max-width="1000" height="auto" width="100%" :draggable="false" @opened="myEvent.createTrajet()" :adaptive="true">
            @include('admin.transfert-voyage.trajet-transfert-voyage.create')
        </modal>
    </div>
</trajet-transfert-voyage-listing>
