...
<type-transfert-voyage-listing :data="{{ $data->toJson() }}" :url="'{{ url('admin/type-transfert-voyages') }}'" :action="'{{ url('admin/type-transfert-voyages') }}'" :urlprestataire="'{{url('admin/prestataires')}}'" :urlbase="'{{base_url('')}}'" :urlasset="'{{asset('')}}'" inline-template>
    <div style="display: contents;position:relative">
            ...
            <div class="col">
                <div class="card position-relative">
                    <div class="card-header">
                        ...
                        <h3>{{ trans('admin.type-transfert-voyage.actions.index') }}</h3>
                        <a class="btn btn-primary btn-sm pull-right m-b-0" href="#" @click.prevent="createType($event,'{{ url('admin/type-transfert-voyages/create')}}')" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.type-transfert-voyage.actions.create') }}</a>
                    </div>
                    <div class="card-body" v-cloak>
                        <div class="card-block position-relative">
                                ...
                                <table class="table table-hover table-listing mt-5">
                                    <thead>
                                        <tr>
                                            ...
                                            <th is='sortable' :column="'id'">{{ trans('admin.type-transfert-voyage.columns.id') }}</th>
                                            <th is='sortable' :column="'titre'">{{ trans('admin.type-transfert-voyage.columns.titre') }}</th>
                                            <th is='sortable' :column="'nombre_min'">{{ trans('admin.type-transfert-voyage.columns.nombre_min') }}</th>
                                            <th is='sortable' :column="'nombre_max'">{{ trans('admin.type-transfert-voyage.columns.nombre_max') }}</th>
                                            <th is='sortable' :column="'prestataire_id'">{{ trans('admin.type-transfert-voyage.columns.prestataire_id') }}</th>
                                            <th></th>
                                        </tr>
                                        <tr v-show="(clickedBulkItemsCount > 0) || isClickedAll">
                                            <td class="bg-bulk-info d-table-cell text-center" colspan="8">
                                                <span class="align-middle font-weight-light text-dark">{{ trans('admin-base.listing.selected_items') }} @{{ clickedBulkItemsCount }}. <a href="#" class="text-primary" @click.prevent="onBulkItemsClickedAll('/admin/type-transfert-voyages')" v-if="(clickedBulkItemsCount < pagination.state.total)"> <i class="fa" :class="bulkCheckingAllLoader ? 'fa-spinner' : ''"></i> {{ trans('admin-base.listing.check_all_items') }} @{{ pagination.state.total }}</a> <span class="text-primary">|</span> <a href="#" class="text-primary" @click.prevent="onBulkItemsClickedAllUncheck()">{{ trans('admin-base.listing.uncheck_all_items') }}</a> </span>

                                                <span class="pull-right pr-2">
                                                    <button class="btn btn-sm btn-danger pr-3 pl-3" @click.prevent="bulkDelete(urlbase+'/admin/type-transfert-voyages/bulk-destroy')">{{ trans('admin-base.btn.delete') }}</button>
                                                </span>
                                    ...
                                    <tbody>
                                        <tr v-for="(item, index) in collection" :key="item.id" :class="bulkItems[item.id] ? 'bg-bulk' : ''">
                                            ...
                                            <td>
                                                <div class="link" @click.prevent="detail($event,item.id)"></div>
                                                @{{ item.nombre_min }} Pers.
                                            </td>
                                            <td>
                                                <div class="link" @click.prevent="detail($event,item.id)"></div>
                                                @{{ item.nombre_max }} Pers.
                                            </td>
                                            <td>
                                                <div class="link" @click.prevent="detail($event,item.id)"></div>
                                            ...
                                            <td>
                                                <div class="row no-gutters">
                                                    ...
                                                    <div class="col-auto">
                                                        <a @contextmenu.prevent="" class="btn btn-sm btn-info" href="#" @click.prevent="editType($event,item.resource_url + '/edit')" title="{{ trans('admin-base.btn.edit') }}" role="button"><i class="fa fa-edit"></i></a>
                                                    </div>
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <a class="btn btn-primary" href="#" @click.prevent="createType($event,'{{ url('admin/type-transfert-voyages/create')}}')" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.type-transfert-voyage.actions.create') }}</a>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <modal name="edit_type" :scrollable="true" :min-width="320" :max-width="1000" height="auto" width="100%" :draggable="false" @opened="myEvent.editType()" :adaptive="true">
            @include('admin.transfert-voyage.type-transfert-voyage.edit')
        </modal>

        <modal name="create_type" :click-to-close="false" :scrollable="true" :min-width="320" :max-width="1000" height="auto" width="100%" :draggable="false" :adaptive="true">
            @include('admin.transfert-voyage.type-transfert-voyage.create')
        </modal>
