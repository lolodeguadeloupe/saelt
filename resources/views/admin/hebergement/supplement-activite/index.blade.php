...
@section('title', trans('admin.supplement-activite.actions.index'))

@section('body')

<supplement-activite-listing :data="{{ $data->toJson() }}" :url="'{{ url('admin/supplement-activites?heb='.$hebergement->id) }}'" :action="'{{ url('admin/supplement-activites?heb='.$hebergement->id) }}'" :appliquer="{{$supp_appliquer}}" :urlchambre="'{{url('admin/chambres?heb='.$hebergement->id)}}'" :urlbase="'{{base_url('')}}'" :urlasset="'{{asset('')}}'" inline-template>
            ...
            <div class="col-10 content-_nav">
                <div class="card position-relative">
                    <div class="card-header">
                        ...
                        <h3>{{ trans('admin.supplement-activite.actions.index') }}</h3>
                        <a class="btn btn-primary btn-sm pull-right m-b-0" href="#" @click.prevent="createSupActivite($event,'{{url('admin/supplement-activites/create?heb='.$hebergement->id)}}')" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.supplement-activite.actions.create') }}</a>
                    </div>
                                ...
                                <table class="table table-hover table-listing mt-5">
                                    <thead>
                                        <tr>
                                            ...
                                            <th is='sortable' :column="'id'">{{ trans('admin.supplement-activite.columns.id') }}</th>
                                            <th is='sortable' :column="'titre'">{{ trans('admin.supplement-activite.columns.titre') }}</th>
                                            <th is='sortable' :column="'prestataire_id'">{{ trans('admin.prestataire.title') }}</th>
                                            <th is='sortable' :column="'regle_tarif'">{{ trans('admin.supplement-activite.columns.regle_tarif') }}</th>
                                            <th>{{ trans('admin.supplement-activite.columns.type_personne') }}</th>
                                        ...
                                        <tr v-show="(clickedBulkItemsCount > 0) || isClickedAll">
                                            <td class="bg-bulk-info d-table-cell text-center" colspan="9">
                                                <span class="align-middle font-weight-light text-dark">{{ trans('admin-base.listing.selected_items') }} @{{ clickedBulkItemsCount }}. <a href="#" class="text-primary" @click.prevent="onBulkItemsClickedAll('/admin/supplement-activites')" v-if="(clickedBulkItemsCount < pagination.state.total)"> <i class="fa" :class="bulkCheckingAllLoader ? 'fa-spinner' : ''"></i> {{ trans('admin-base.listing.check_all_items') }} @{{ pagination.state.total }}</a> <span class="text-primary">|</span> <a href="#" class="text-primary" @click.prevent="onBulkItemsClickedAllUncheck()">{{ trans('admin-base.listing.uncheck_all_items') }}</a> </span>

                                                <span class="pull-right pr-2">
                                                    <button class="btn btn-sm btn-danger pr-3 pl-3" @click.prevent="bulkDelete(urlbase+'/admin/supplement-activites/bulk-destroy')">{{ trans('admin-base.btn.delete') }}</button>
                                                </span>
                                        ...
                                        <tr v-for="(item, index) in collection" :key="item.id" :class="bulkItems[item.id] ? 'bg-bulk' : ''">
                                            ...
                                            <td style="display: table-cell !important;">
                                                <div class="row no-gutters">
                                                    <div class="col-auto">
                                                        <a class="btn btn-sm btn-info" @click.prevent="editSupActivite($event,item.resource_url + '/edit')" href="#" title="{{ trans('admin-base.btn.edit') }}" role="button"><i class="fa fa-edit"></i></a>
                                                    </div>
...
-->
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <a class="btn btn-primary" href="#" @click.prevent="createSupActivite($event,'{{url('admin/supplement-activites/create?heb='.$hebergement->id)}}')" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.supplement-activite.actions.create') }}</a>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <modal name="edit_sup_activite" :scrollable="true" :min-width="320" :max-width="1000" height="auto" width="100%" :draggable="true" @opened="myEvent.editSupActivite()" :adaptive="true">
            @include('admin.hebergement.supplement-activite.edit')
        </modal>
        <modal name="create_sup_activite" :scrollable="true" :min-width="320" :max-width="1000" height="auto" width="100%" :draggable="true" @opened="myEvent.createSupActivite()" :adaptive="true">
            @include('admin.hebergement.supplement-activite.create')
        </modal>
        <modal name="edit_tarif_personne" :scrollable="true" :min-width="320" :max-width="1000" height="auto" width="100%" :draggable="true" @opened="myEvent.editTarifPersonne()" :adaptive="true">
            <form class="form-horizontal form-edit" id="edit-tarif-form" method="post" @submit.prevent="updateTarifForm($event,'edit_tarif_personne')" :action="actionEditTarifPersonne" novalidate>
                ...
                <div class="card-body">
                    ...
                    <div class="form-group row align-items-center">
                        <label for="montant" class="col-form-label text-md-right required" :class="isFormLocalized ? 'col-md-4' : 'col-md-2'">{{ trans('admin.supplement-activite.columns.tarif') }}</label>
                        <div :class="isFormLocalized ? 'col-md-4' : 'col-md-9 col-xl-8'">
