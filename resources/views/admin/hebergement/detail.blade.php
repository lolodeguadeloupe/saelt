...
@section('body')
<detail-listing :detail="{{$data}}" :datacalendar="{{$calendar->toJson()}}" :url="'{{ $hebergement->resource_url }}'" :action="'{{url('admin/hebergements') }}'" :chambre="'{{url('admin/chambres')}}'" :urlTypeHeb="'{{url('admin/type-hebergemets')}}'" :urlbase="'{{base_url('')}}'"  :urlasset="'{{asset('')}}'" :urlcalendar="'{{url('admin/event-date-heures')}}'" inline-template>
    <div style="display: contents;position:relative">
                    ...
                    <div class="card-body" v-cloak style="position: relative;" v-for="(item,index) in data_detail" :key="item.id">
                        ...
                        <div class="row">
                            <div class="col-12" v-if="pagination_[0]">
                                ...
                                <div class="form-_label">
                                    <label for="type" class="col-form-label text-md-right col-md-3">{{trans('admin.hebergement.columns.devises_id')}} </label>
                                    <span class="form-control  cliquable" @click.prevent="editHeb($event,item.resource_url + '/edit')">@{{item.devises.titre}}</span>
                                </div>
                                <div class="form-_label">
                                    <label for="type" class="col-form-label text-md-right col-md-3">{{trans('admin.hebergement.columns.adresse')}} </label>
                                    <span class="form-control  cliquable" @click.prevent="editHeb($event,item.resource_url + '/edit')">@{{item.adresse}}</span>
                                </div>
                                <div class="form-_label">
                                    <label for="type" class="col-form-label text-md-right col-md-3">{{trans('admin.hebergement.columns.ville_id')}} </label>
                                ...
                                <div class="form-_label">
                                    ...
                                    <span class="form-control  cliquable" @click.prevent="editHeb($event,item.resource_url + '/edit')">@{{item.description}}</span>
                                </div>
                                <div class="form-_label">
                                ...
                                <div class="form-_label">
                                    ...
                                    <span class="form-control  cliquable" style="padding-right: 0.75rem !important;padding-left: 0.75rem !important;" @click.prevent="editHeb($event,item.resource_url + '/edit')">
                                        <span v-for="(_image, _index_img) in item.image" style="display: inline-block; width: calc((50% + 0.5em) - 0.75rem); margin: 0;padding: 5px;">
                                            <img :src="urlbase +'/'+ _image.name" alt="" style="width: 100%;height: auto; margin:auto">
                                        </span>
        ...
        <modal name="edit_hebergement" :min-width="320" :scrollable="true" :max-width="1000" height="auto" width="100%" :draggable="true" @opened="myEvent.editHeb()" :adaptive="true">
        ...
        <modal name="create_devises" :scrollable="true" :min-width="320" :max-width="1000" height="auto" width="100%" :draggable="true" :adaptive="true">
            @include('admin.hebergement.hebergement.devises')
        </modal>
