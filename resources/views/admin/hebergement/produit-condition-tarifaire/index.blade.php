...
<produit-condition-tarifaire-listing :data="{{ $data->toJson() }}" :url="'{{ url('admin/hebergement-produit-condition-tarifaires/'.$hebergement->id.'?heb='.$hebergement->id) }}'" :action="'{{ url('admin/hebergement-produit-condition-tarifaires') }}'" :model="{{$hebergement->toJson()}}" :urlbase="'{{base_url('')}}'" :urlasset="'{{asset('')}}'" inline-template>
            ...
            <div class="col-10 content-_nav">
                <div class="card">
                    <div class="card-header">
                        ...
                        <a class="btn btn-primary btn-sm pull-right m-b-0" href="#" @click.prevent="editCondition($event,'{{url('admin/hebergement-produit-condition-tarifaires/'.$hebergement->id)}}')" role="button" v-if="is_edit==false"><i class="fa fa-pencil"></i>&nbsp; {{ trans('admin.produit-condition-tarifaire.actions.edit') }}</a>
                    </div>
