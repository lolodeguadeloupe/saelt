...
@section('title', trans('admin.type-hebergement.actions.index'))

@section('body')

<type-hebergement-listing :data="{{ $data->toJson() }}" :url="'{{ url('admin/type-hebergements') }}'" :urlbase="'{{base_url('')}}'" :urlasset="'{{asset('')}}'" inline-template>
    <diV style="display:contents; position:relative">
            ...
            <div class="col">
                <div class="card position-relative">
                    <div class="card-header">
                        ...
                        <h3>{{ trans('admin.type-hebergement.actions.index') }}</h3>
                        <a class="btn btn-primary btn-sm pull-right m-b-0" href="#" @click.prevent="createType($event,'{{url('admin/type-hebergements/create')}}')" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.type-hebergement.actions.create') }}</a>
                    </div>
                    <div class="card-body" v-cloak>
                        <div class="card-block position-relative">
                                ...
                                <table class="table table-hover table-listing mt-5">
                                    <thead>
                                        <tr>
                                            ...
                                            <th is='sortable' :column="'id'">{{ trans('admin.type-hebergement.columns.id') }}</th>
                                            <th is='sortable' :column="'name'">{{ trans('admin.type-hebergement.columns.name') }}</th>
                                            <th is='sortable' :column="'description'">{{ trans('admin.type-hebergement.columns.description') }}</th>

                                            <th></th>
                                        </tr>
                                        <tr v-show="(clickedBulkItemsCount > 0) || isClickedAll">
                                            <td class="bg-bulk-info d-table-cell text-center" colspan="5">
                                                <span class="align-middle font-weight-light text-dark">{{ trans('admin-base.listing.selected_items') }} @{{ clickedBulkItemsCount }}. <a href="#" class="text-primary" @click.prevent="onBulkItemsClickedAll('/admin/type-hebergements')" v-if="(clickedBulkItemsCount < pagination.state.total)"> <i class="fa" :class="bulkCheckingAllLoader ? 'fa-spinner' : ''"></i> {{ trans('admin-base.listing.check_all_items') }} @{{ pagination.state.total }}</a> <span class="text-primary">|</span> <a href="#" class="text-primary" @click.prevent="onBulkItemsClickedAllUncheck()">{{ trans('admin-base.listing.uncheck_all_items') }}</a> </span>

                                                <span class="pull-right pr-2">
                                                    <button class="btn btn-sm btn-danger pr-3 pl-3" @click.prevent="bulkDelete(urlbase+'/admin/type-hebergements/bulk-destroy')">{{ trans('admin-base.btn.delete') }}</button>
                                                </span>
                                    ...
                                    <tbody>
                                        <tr v-for="(item, index) in collection" :key="item.id" :class="bulkItems[item.id] ? 'bg-bulk' : ''">
                                            ...
                                            <td>
                                                <div class="link" @click.prevent="editType($event,item.resource_url + '/edit')"></div>
                                                @{{ item.id }}
                                            </td>
                                            <td>
                                                <div class="link" @click.prevent="editType($event,item.resource_url + '/edit')"></div>
                                            ...
                                            <td>
                                                <div class="row no-gutters">
                                                    <div class="col-auto">
                                                        <a class="btn btn-sm btn-info" @click.prevent="editType($event,item.resource_url + '/edit')" href="#" title="{{ trans('admin-base.btn.edit') }}" role="button"><i class="fa fa-edit"></i></a>
                                                    </div>
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <a class="btn btn-primary" href="#" @click.prevent="createType($event,'{{url('admin/type-hebergements/create')}}')" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.type-hebergement.actions.create') }}</a>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <modal name="create_type" :scrollable="true" :min-width="320" :max-width="1000" height="auto" width="100%" :draggable="true" @opened="myEvent.createType()" :adaptive="true">
            @include('admin.hebergement.type-hebergement.create')
        </modal>
        <modal name="edit_type" :scrollable="true" :min-width="320" :max-width="1000" height="auto" width="100%" :draggable="true" @opened="myEvent.editType()" :adaptive="true">
            @include('admin.hebergement.type-hebergement.edit')
        </modal>
    </diV>
</type-hebergement-listing>
