...
<service-aeroport-listing :data="{{ $data->toJson() }}" :url="'{{ url('admin/service-aeroports') }}'" :action="'{{ url('admin/service-aeroports') }}'" :urlbase="'{{base_url('')}}'" :urlasset="'{{asset('')}}'" :urlcalendar="'{{url('admin/event-date-heures')}}'" inline-template>
            ...
            <div class="col">
                <div class="card position-relative">
                    <div class="card-header">
                        ...
                        <a class="btn btn-primary btn-sm pull-right m-b-0" href="#" @click.prevent="createService($event,'{{url('admin/service-aeroports/create')}}')" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.service-aeroport.actions.create') }}</a>
                    </div>
                    <div class="card-body" v-cloak>
                        <div class="card-block position-relative">
                                ...
                                <table class="table table-hover table-listing mt-5">
                                    <thead>
                                        <tr>
                                            ...
                                            <th is='sortable' :column="'id'">{{ trans('admin.service-aeroport.columns.id') }}</th>
                                            <th is='sortable' :column="'code_service'">{{ trans('admin.service-aeroport.columns.code_service') }}</th>
                                            <th is='sortable' :column="'name'">{{ trans('admin.service-aeroport.columns.name') }}</th>
                                            <th is='sortable' :column="'adresse'">{{ trans('admin.service-aeroport.columns.adresse') }}</th>
                                            <th is='sortable' :column="'phone'">{{ trans('admin.service-aeroport.columns.phone') }}</th>
                                            <th is='sortable' :column="'email'">{{ trans('admin.service-aeroport.columns.email') }}</th>
                                            <th is='sortable' :column="'ville_id'">{{ trans('admin.service-aeroport.columns.ville_id') }}</th>

                                            <th></th>
                                        </tr>
                                        <tr v-show="(clickedBulkItemsCount > 0) || isClickedAll">
                                            <td class="bg-bulk-info d-table-cell text-center" colspan="9">
                                                <span class="align-middle font-weight-light text-dark">{{trans('admin-base.listing.selected_items') }} @{{ clickedBulkItemsCount }}. <a href="#" class="text-primary" @click.prevent="onBulkItemsClickedAll('/admin/service-aeroports')" v-if="(clickedBulkItemsCount < pagination.state.total)"> <i class="fa" :class="bulkCheckingAllLoader ? 'fa-spinner' : ''"></i> {{trans('admin-base.listing.check_all_items') }} @{{ pagination.state.total }}</a> <span class="text-primary">|</span> <a href="#" class="text-primary" @click.prevent="onBulkItemsClickedAllUncheck()">{{trans('admin-base.listing.uncheck_all_items') }}</a> </span>

                                                <span class="pull-right pr-2">
                                                    <button class="btn btn-sm btn-danger pr-3 pl-3" @click.prevent="bulkDelete(urlbase+'/admin/service-aeroports/bulk-destroy')">{{trans('admin-base.btn.delete') }}</button>
                                                </span>
                                        ...
                                        <tr v-for="(item, index) in collection" :key="item.id" :class="bulkItems[item.id] ? 'bg-bulk' : ''">
                                            ...
                                            <td>
                                                <div class="row no-gutters">
                                                    ...
                                                    <div class="col-auto">
                                                        <a class="btn btn-sm btn-info" @click.prevent="editService($event,action+'/'+item.id + '/edit')" href="#" title="{{trans('admin-base.btn.edit') }}" role="button"><i class="fa fa-edit"></i></a>
                                                    </div>
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <a class="btn btn-primary" href="#" @click.prevent="createService($event,'{{url('admin/service-aeroports/create')}}')" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.service-aeroport.actions.create') }}</a>
                            </div>
                        </div>
                    </div>
                </div>
