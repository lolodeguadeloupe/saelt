...
<vehicule-location-listing :data="{{ $data->toJson() }}" :url="'{{ url('admin/vehicule-locations') }}'" :action="'{{url('admin/vehicule-locations') }}'" :urlbase="'{{base_url('')}}'" :urlasset="'{{asset('')}}'" :urlvehiculelocationwithprestataire="'{{url('admin/vehicule-locations/prestataire')}}'" :urlprestataire="'{{url('admin/prestataires')}}'" :urlcalendar="'{{url('admin/event-date-heures')}}'" :urltaxe="'{{url('admin/taxes')}}'" inline-template>
    ...
    <diV style="display:contents; position:relative">
            ...
            <div class="col">
                <div class="card position-relative">
                    <div class="card-header">
                        ...
                        <h3>{{ trans('admin.vehicule-location.actions.index') }}</h3>
                        <a class="btn btn-primary btn-sm pull-right m-b-0" href="#" @click.prevent="createVehicule($event,'{{ url('admin/vehicule-locations/create') }}')" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.vehicule-location.actions.create') }}</a>
                    </div>
                    <div class="card-body" v-cloak>
                        <div class="card-block position-relative">
                                ...
                                <table class="table table-hover table-listing mt-5">
                                    <thead>
                                        <tr>
                                            ...
                                            <th is='sortable' :column="'id'">{{ trans('admin.vehicule-location.columns.id') }}</th>
                                            <th is='sortable' :column="'titre'">{{ trans('admin.vehicule-location.columns.titre') }}</th>
                                            <th is='sortable' :column="'immatriculation'">{{ trans('admin.vehicule-location.columns.immatriculation') }}</th>
                                            <th is='sortable' :column="'marque_vehicule_id'">{{ trans('admin.vehicule-location.columns.marque') }}</th>
                                            <th is='sortable' :column="'modele_vehicule_id'">{{ trans('admin.vehicule-location.columns.modele') }}</th>
                                            <th is='sortable' :column="'categorie_vehicule.titre'">{{ trans('admin.vehicule-location.columns.categorie_vehicule') }}</th>
                                            <th is='sortable' :column="'famille_vehicule.titre'">{{ trans('admin.vehicule-location.columns.famille_vehicule') }}</th>
                                            <th is='sortable' :column="'status'">{{ trans('admin.vehicule-location.columns.status') }}</th>
                                            <th is='sortable' :column="'duration_min'">{{ trans('admin.vehicule-location.columns.duration_min') }}</th>
                                            <th is='sortable' :column="'prestataire.name'">{{ trans('admin.vehicule-location.columns.prestataire_id') }}</th>
                                            <th></th>
                                        </tr>
                                        <tr v-show="(clickedBulkItemsCount > 0) || isClickedAll">
                                            <td class="bg-bulk-info d-table-cell text-center" colspan="12">
                                                ...
                                                <span class="pull-right pr-2">
                                                    <button class="btn btn-sm btn-danger pr-3 pl-3" @click.prevent="bulkDelete(urlbase+'/admin/vehicule-locations/bulk-destroy')">{{ trans('admin-base.btn.delete') }}</button>
                                                </span>
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <a class="btn btn-primary" href="#" @click.prevent="createVehicule($event,'{{ url('admin/vehicule-locations/create') }}')" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.vehicule-location.actions.create') }}</a>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <modal name="edit_vehicule" :scrollable="true" :min-width="320" :max-width="1000" height="auto" width="100%" :draggable="false" @opened="myEvent.editVehicule()" :adaptive="true">
            @include('admin.location-vehicule.vehicule-location.edit')
        </modal>
        ...
        <modal name="create_vehicule" :click-to-close="false" :scrollable="true" :min-width="320" :max-width="1000" height="auto" width="100%" :draggable="false" :adaptive="true">
            @include('admin.location-vehicule.vehicule-location.create')
        </modal>
        ...
        <modal name="create_categorie_vehicule" :class="'min-width'" :click-to-close="false" :scrollable="true" :min-width="320" :max-width="1000" height="auto" width="100%" :draggable="false" :adaptive="true">
            @include('admin.location-vehicule.vehicule-location.categorie')
        </modal>
        ...
        <modal name="create_famille_vehicule" :class="'min-width'" :click-to-close="false" :scrollable="true" :min-width="320" :max-width="1000" height="auto" width="100%" :draggable="false" :adaptive="true">
            @include('admin.location-vehicule.vehicule-location.famille')
        </modal>
        ...
        <modal name="create_marque_vehicule" :scrollable="true" :min-width="320" :max-width="1000" height="auto" width="100%" :draggable="true" :adaptive="true">
            @include('admin.location-vehicule.vehicule-location.marque')
        </modal>
        <modal name="create_modele_vehicule" :scrollable="true" :min-width="320" :max-width="1000" height="auto" width="100%" :draggable="true" :adaptive="true">
            @include('admin.location-vehicule.vehicule-location.modele')
        </modal>
