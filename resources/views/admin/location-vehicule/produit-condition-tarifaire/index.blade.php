...
<produit-condition-tarifaire-listing :data="{{ $data->toJson() }}" :url="'{{ url('admin/vehicule-locations-produit-condition-tarifaires/'.$locationVehicule->id.'?location='.$locationVehicule->id) }}'" :action="'{{ url('admin/vehicule-locations-produit-condition-tarifaires') }}'" :model="{{$locationVehicule->toJson()}}" :urlbase="'{{base_url('')}}'" :urlasset="'{{asset('')}}'" inline-template>
            ...
            <div class="col-10 content-_nav">
                <div class="card">
                    <div class="card-header">
                        ...
                        <a class="btn btn-primary btn-sm pull-right m-b-0" href="#" @click.prevent="editCondition($event,'{{url('admin/vehicule-locations-produit-condition-tarifaires/'.$locationVehicule->id)}}')" role="button" v-if="is_edit==false"><i class="fa fa-pencil"></i>&nbsp; {{ trans('admin.produit-condition-tarifaire.actions.edit') }}</a>
                    </div>
