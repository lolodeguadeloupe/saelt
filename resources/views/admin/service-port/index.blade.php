...
@section('title', trans('admin.service-port.actions.index'))

@section('body')

<service-port-listing :data="{{ $data->toJson() }}" :url="'{{ url('admin/service-ports') }}'" :action="'{{ url('admin/service-ports') }}'" :urlbase="'{{base_url('')}}'" :urlasset="'{{asset('')}}'" :urlcalendar="'{{url('admin/event-date-heures')}}'" inline-template>
    <div style="display: contents;position:relative">
            ...
            <div class="col">
                <div class="card position-relative">
                    <div class="card-header">
                        ...
                        <h3>{{ trans('admin.service-port.actions.index') }}</h3>
                        <a class="btn btn-primary btn-sm pull-right m-b-0" href="#" @click.prevent="createService($event,'{{url('admin/service-ports/create')}}')" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.service-port.actions.create') }}</a>
                    </div>
                    <div class="card-body" v-cloak>
                        <div class="card-block position-relative">
                                ...
                                <table class="table table-hover table-listing mt-5">
                                    <thead>
                                        <tr>
                                            ...
                                            <th is='sortable' :column="'id'">{{ trans('admin.service-port.columns.id') }}</th>
                                            <th is='sortable' :column="'code_service'">{{ trans('admin.service-port.columns.code_service') }}</th>
                                            <th is='sortable' :column="'name'">{{ trans('admin.service-port.columns.name') }}</th>
                                            <th is='sortable' :column="'adresse'">{{ trans('admin.service-port.columns.adresse') }}</th>
                                            <th is='sortable' :column="'phone'">{{ trans('admin.service-port.columns.phone') }}</th>
                                            <th is='sortable' :column="'email'">{{ trans('admin.service-port.columns.email') }}</th>
                                            <th is='sortable' :column="'ville_id'">{{ trans('admin.service-port.columns.ville_id') }}</th>

                                            <th></th>
                                        </tr>
                                        <tr v-show="(clickedBulkItemsCount > 0) || isClickedAll">
                                            <td class="bg-bulk-info d-table-cell text-center" colspan="9">
                                                <span class="align-middle font-weight-light text-dark">{{ trans('brackets/admin-ui::admin.listing.selected_items') }} @{{ clickedBulkItemsCount }}. <a href="#" class="text-primary" @click.prevent="onBulkItemsClickedAll('/admin/service-ports')" v-if="(clickedBulkItemsCount < pagination.state.total)"> <i class="fa" :class="bulkCheckingAllLoader ? 'fa-spinner' : ''"></i> {{ trans('brackets/admin-ui::admin.listing.check_all_items') }} @{{ pagination.state.total }}</a> <span class="text-primary">|</span> <a href="#" class="text-primary" @click.prevent="onBulkItemsClickedAllUncheck()">{{ trans('brackets/admin-ui::admin.listing.uncheck_all_items') }}</a> </span>

                                                <span class="pull-right pr-2">
                                                    <button class="btn btn-sm btn-danger pr-3 pl-3" @click.prevent="bulkDelete(urlbase+'/admin/service-ports/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                                </span>
                                        ...
                                        <tr v-for="(item, index) in collection" :key="item.id" :class="bulkItems[item.id] ? 'bg-bulk' : ''">
                                            ...
                                            <td>
                                                <div class="row no-gutters">
                                                    ...
                                                    <div class="col-auto">
                                                        <a class="btn btn-sm btn-info" @click.prevent="editService($event,action+'/'+item.id + '/edit')" href="#" title="{{ trans('brackets/admin-ui::admin.btn.edit') }}" role="button"><i class="fa fa-edit"></i></a>
                                                    </div>
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <a class="btn btn-primary" href="#" @click.prevent="createService($event,'{{url('admin/service-ports/create')}}')" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.service-port.actions.create') }}</a>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <modal name="edit_service" :min-width="320" :scrollable="true" :max-width="1000" height="auto" width="100%" :draggable="true" @opened="myEvent.editService()" :adaptive="true">
            @include('admin.service-port.edit')
        </modal>
        ...
        <modal name="calendar" :class="'min-width'" :click-to-close="false" :scrollable="true" :min-width="320" :max-width="1000" height="auto" width="100%" :draggable="true" :adaptive="true">
            @include('admin.service-port.calendar')
        </modal>
