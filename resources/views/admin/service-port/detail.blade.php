...
@section('body')
<detail-service-port-listing :detail="{{$data}}" :datacalendar="{{$servicePort->calendar->toJson()}}" :url="'{{ $servicePort->resource_url }}'" :action="'{{url('admin/service-ports') }}'" :urlbase="'{{base_url('')}}'"  :urlasset="'{{asset('')}}'" :urlcalendar="'{{url('admin/event-date-heures')}}'" inline-template>
                        ...
                        <div class="row">
                            <div class="col-12">
                                ...
                                <div class="form-_label">
                                    <label for="title" class="col-form-label text-md-right col-md-3">{{ trans('admin.service-port.columns.code_service') }} </label>
                                    <span class="form-control cliquable" @click.prevent="editService($event,item.resource_url + '/edit')">@{{item.code_service}}</span>
                                </div>

                                <div class="form-_label">
                                    <label for="immatriculation" class="col-form-label text-md-right col-md-3">{{ trans('admin.service-port.columns.name') }} </label>
                                    <span class="form-control cliquable" @click.prevent="editService($event,item.resource_url + '/edit')">@{{item.name}}</span>
                                </div>
                                <div class="form-_label">
                                    <label for="marque" class="col-form-label text-md-right col-md-3">{{ trans('admin.service-port.columns.adresse') }} </label>
                                    <span class="form-control cliquable" @click.prevent="editService($event,item.resource_url + '/edit')">@{{item.adresse}}</span>
                                </div>
                                <div class="form-_label">
                                    <label for="modele" class="col-form-label text-md-right col-md-3">{{ trans('admin.service-port.columns.phone') }} </label>
                                    <span class="form-control cliquable" @click.prevent="editService($event,item.resource_url + '/edit')">@{{item.phone}}</span>
                                </div>

                                <div class="form-_label">
                                    <label for="categorie_vehicule_id" class="col-form-label text-md-right col-md-3">{{ trans('admin.service-port.columns.email') }} </label>
                                    <span class="form-control cliquable" @click.prevent="editService($event,item.resource_url + '/edit')">@{{item.email}}</span>
                                </div>

                                <div class="form-_label">
                                    <label for="date_ouverture" class="col-form-label text-md-right col-md-3">{{ trans('admin.service-port.columns.ville_id') }} </label>
                                    <span class="form-control cliquable" @click.prevent="editService($event,item.resource_url + '/edit')">@{{item.ville.name}}</span>
                                </div>

                                <div class="form-_label">
                                    <label for="name" class="col-form-label text-md-right col-md-3">{{trans('admin.service-port.columns.logo')}} </label>
                                    <span class="form-control  cliquable" style="padding-right: 0.75rem !important;padding-left: 0.75rem !important;" @click.prevent="editService($event,item.resource_url + '/edit')">
                                        <span v-if="item.logo" style="display: inline-block; width: calc((50% + 0.5em) - 0.75rem); margin: 0;padding: 5px;">
                                            <img :src="$isBase64(item.logo)?item.logo:`${urlasset}/${item.logo}`" alt="" style="width: 100px;height: auto; margin:auto">
                                        </span>
                                ...
                                <div class="form-_label">
                                    <label for="calendar" class="col-form-label text-md-right col-md-3">{{trans('admin.service-port.columns.calendar')}} </label>
