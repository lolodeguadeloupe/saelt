...
@section('title', trans('admin.taxe.actions.index'))

@section('body')

<taxe-listing :data="{{ $data->toJson() }}" :url="'{{ url('admin/taxes') }}'" :action="'{{ url('admin/taxes') }}'" :urlbase="'{{base_url('')}}'" :urlasset="'{{asset('')}}'" inline-template>
    ...
    <div style="display: contents;position:relative">
            ...
            <div class="col-12">
                <div class="card position-relative">
                    <div class="card-header">
                        <i class="fa fa-align-justify"></i> {{ trans('admin.taxe.actions.index') }}
                        <a class="btn btn-primary btn-sm pull-right m-b-0" href="#" @click.prevent="createTaxe($event,'{{ url('admin/taxes/create')}}')" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.taxe.actions.create') }}</a>
                    </div>
                    <div class="card-body" v-cloak>
                        <div class="card-block position-relative">
                                ...
                                <table class="table table-hover table-listing mt-5">
                                    <thead>
                                        <tr>
                                            ...
                                            <th is='sortable' :column="'id'">{{ trans('admin.taxe.columns.id') }}</th>
                                            <th is='sortable' :column="'titre'">{{ trans('admin.taxe.columns.titre') }}</th>
                                            <th is='sortable' :column="'valeur_pourcent'">{{ trans('admin.taxe.columns.valeur_pourcent') }}</th>
                                            <th is='sortable' :column="'valeur_devises'">{{ trans('admin.taxe.columns.valeur_devises') }}</th>

                                            <th></th>
                                        </tr>
                                        <tr v-show="(clickedBulkItemsCount > 0) || isClickedAll">
                                            <td class="bg-bulk-info d-table-cell text-center" colspan="6">
                                                <span class="align-middle font-weight-light text-dark">{{ trans('admin-base.listing.selected_items') }} @{{ clickedBulkItemsCount }}. <a href="#" class="text-primary" @click.prevent="onBulkItemsClickedAll('/admin/taxes')" v-if="(clickedBulkItemsCount < pagination.state.total)"> <i class="fa" :class="bulkCheckingAllLoader ? 'fa-spinner' : ''"></i> {{ trans('admin-base.listing.check_all_items') }} @{{ pagination.state.total }}</a> <span class="text-primary">|</span> <a href="#" class="text-primary" @click.prevent="onBulkItemsClickedAllUncheck()">{{ trans('admin-base.listing.uncheck_all_items') }}</a> </span>

                                                <span class="pull-right pr-2">
                                                    <button class="btn btn-sm btn-danger pr-3 pl-3" @click.prevent="bulkDelete(urlbase+'/admin/taxes/bulk-destroy')">{{ trans('admin-base.btn.delete') }}</button>
                                                </span>
                                        ...
                                        <tr v-for="(item, index) in collection" :key="item.id" :class="bulkItems[item.id] ? 'bg-bulk' : ''">
                                            ...
                                            <td>
                                                <div class="row no-gutters">
                                                    <div class="col-auto">
                                                        <a class="btn btn-sm btn-info" href="#" @click.prevent="editTaxe($event,item.resource_url + '/edit')" title="{{ trans('admin-base.btn.edit') }}" role="button"><i class="fa fa-edit"></i></a>
                                                    </div>
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <a class="btn btn-primary" href="#" @click.prevent="createTaxe($event,'{{ url('admin/taxes/create')}}')" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.taxe.actions.create') }}</a>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <modal name="edit_taxe" :scrollable="true" :min-width="320" :max-width="1000" height="auto" width="100%" :draggable="true" @opened="myEvent.editTaxe()" :adaptive="true">
            @include('admin.taxe.edit')
        </modal>
