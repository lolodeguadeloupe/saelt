...
<allotement-listing :data="{{ $data->toJson() }}" :action="'{{ url('admin/allotements') }}'" :url="'{{ url('admin/allotements') }}'" :urlcompagnie="'{{url('admin/compagnie-transports')}}'" :urlbase="'{{base_url('')}}'" :urlasset="'{{asset('')}}'" inline-template>
    ...
    <div style="display: contents;position:relative">
            ...
            <div class="col-12">
                <div class="card position-relative">
                    <div class="card-header">
                        ...
                        <h3>{{ trans('admin.allotement.actions.index') }}</h3>
                                    ...
                                    <thead>
                                        ...
                                        <tr v-show="(clickedBulkItemsCount > 0) || isClickedAll">
                                            <td class="bg-bulk-info d-table-cell text-center" colspan="14">
                                                <span class="align-middle font-weight-light text-dark">{{ trans('admin-base.listing.selected_items') }} @{{ clickedBulkItemsCount }}. <a href="#" class="text-primary" @click.prevent="onBulkItemsClickedAll('/admin/allotements')" v-if="(clickedBulkItemsCount < pagination.state.total)"> <i class="fa" :class="bulkCheckingAllLoader ? 'fa-spinner' : ''"></i> {{ trans('admin-base.listing.check_all_items') }} @{{ pagination.state.total }}</a> <span class="text-primary">|</span> <a href="#" class="text-primary" @click.prevent="onBulkItemsClickedAllUncheck()">{{ trans('admin-base.listing.uncheck_all_items') }}</a> </span>

                                                <span class="pull-right pr-2">
                                                    <button class="btn btn-sm btn-danger pr-3 pl-3" @click.prevent="bulkDelete(urlbase+'/admin/allotements/bulk-destroy')">{{ trans('admin-base.btn.delete') }}</button>
                                                </span>
                                        ...
                                        <tr v-for="(item, index) in collection" :key="item.id" :class="bulkItems[item.id] ? 'bg-bulk' : ''">
                                            ...
                                            <td>
                                                <div class="row no-gutters">
                                                    <div class="col-auto">
                                                        <a class="btn btn-sm btn-info" @click.prevent="editAllotement($event,item.resource_url + '/edit')" title="{{ trans('admin-base.btn.edit') }}" role="button"><i class="fa fa-edit"></i></a>
                                                    </div>
        ...
        <modal name="create_allotement" :scrollable="true" :min-width="320" :max-width="1000" height="auto" width="100%" :draggable="true" @opened="myEvent.createAllotement()" :adaptive="true">
            @include('admin.allotement.create')
        </modal>
