...
<saison-listing :data="{{ $data->toJson() }}" :url="'{{ url('admin/saisons?excursion='.$excursion->id) }}'" :action="'{{ url('admin/saisons?excursion='.$excursion->id) }}'" :urlbase="'{{base_url('')}}'" :urlasset="'{{asset('')}}'" inline-template>
    <div style="display: contents;position:relative">
        ...
        <div class="row has--child-nav">
            ...
            <div class="col-10 content-_nav">
                <div class="card position-relative">
                    <div class="card-header">
                        ...
                        <a class="btn btn-primary btn-sm pull-right m-b-0" href="#" @click.prevent="createSaison($event,'{{url('admin/saisons/create')}}')" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.saison.actions.create') }}</a>
                    </div>
                    <div class="card-body" v-cloak>
                        <div class="card-block position-relative">
                                    ...
                                    <thead>
                                        ...
                                        <tr v-show="(clickedBulkItemsCount > 0) || isClickedAll">
                                            <td class="bg-bulk-info d-table-cell text-center" colspan="6">
                                                <span class="align-middle font-weight-light text-dark">{{ trans('admin-base.listing.selected_items') }} @{{ clickedBulkItemsCount }}. <a href="#" class="text-primary" @click.prevent="onBulkItemsClickedAll('/admin/saisons')" v-if="(clickedBulkItemsCount < pagination.state.total)"> <i class="fa" :class="bulkCheckingAllLoader ? 'fa-spinner' : ''"></i> {{ trans('admin-base.listing.check_all_items') }} @{{ pagination.state.total }}</a> <span class="text-primary">|</span> <a href="#" class="text-primary" @click.prevent="onBulkItemsClickedAllUncheck()">{{ trans('admin-base.listing.uncheck_all_items') }}</a> </span>

                                                <span class="pull-right pr-2">
                                                    <button class="btn btn-sm btn-danger pr-3 pl-3" @click.prevent="bulkDelete(urlbase+'/admin/saisons/bulk-destroy')">{{ trans('admin-base.btn.delete') }}</button>
                                                </span>
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <a class="btn btn-primary" href="#" @click.prevent="createSaison($event,'{{url('admin/saison')}}')" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.saison.actions.create') }}</a>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <modal name="edit_saison" :min-width="320" :max-width="1000" height="auto" width="100%" :draggable="true" @opened="myEvent.editSaison()" :adaptive="true">
                @include('admin.excursion.saison.edit')
            </modal>
            <modal name="create_saison" :min-width="320" :max-width="1000" height="auto" width="100%" :draggable="true" @opened="myEvent.createSaison()" :adaptive="true">
                @include('admin.excursion.saison.create')
            </modal>
        </div>
</saison-listing>
